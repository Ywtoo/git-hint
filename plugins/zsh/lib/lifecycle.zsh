...
_githint_init() {
    ...
    # Register widgets (one time only). Without `zle -N githint-esc` the
    # bindkey target does not exist and pressing ESC prints
    # "No such widget `githint-esc'" — leaving any expansion stuck on screen.
...
_githint_enable() {
    ...
    local configured_render
    configured_render="$("$GITHINT_BIN" config get render 2>/dev/null)"
    [[ "$configured_render" == "basic" || "$configured_render" == "ohmyzsh" ]] && GITHINT_RENDER="$configured_render"
