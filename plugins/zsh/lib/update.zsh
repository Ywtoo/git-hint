...
_githint_update() {
    (( PENDING > 0 )) && return
    (( GITHINT_SUPPRESS_UPDATE )) && return
    ...
    local request=$'list\t'"$buffer"$'\t'"$GITHINT_SELECTED"$'\t'"$GITHINT_PROMPT_COL"$'\t'"$GITHINT_RENDER"$'\t'"$PWD"$'\t'"$(_githint_aliases_payload)"
    local result=""
    ...
    if [[ -n "$result" ]]; then
        # POSTDISPLAY starts with a newline (new_postdisplay=$'\n'...), so the
        # first list character sits at ${#buffer} + 1. Counting it, or every
        # highlighted region ends up shifted one column left and the last
        # character of each styled span renders unstyled.
        local base=$(( ${#buffer} + 1 ))
