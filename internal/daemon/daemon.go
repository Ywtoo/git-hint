...
// parseAliasesPayload decodes "name\tdefinition\tname\tdefinition..." into
// a map. Odd trailing tokens (corrupted payload) are ignored. Definitions
// arrive quoted by `alias -L` (name='def'); the surrounding quotes are
// stripped so the definition tokenizes as the real command.
func parseAliasesPayload(payload string) map[string]string {
	...
	for i := 0; i+1 < len(tokens); i += 2 {
		...
		out[name] = unquoteAliasDefinition(tokens[i+1])
	}
	return out
}

// unquoteAliasDefinition removes one level of matching surrounding quotes.
func unquoteAliasDefinition(def string) string {
	if len(def) >= 2 {
...
func dispatch(line string) string {
	...
	case "list":
		// list \t buffer \t selected \t col \t render \t workingDir
		if len(fields) < 5 {
			return ""
		}
		buffer := fields[1]
		...
		workingDir := ""
		if len(fields) >= 6 {
			workingDir = strings.TrimSpace(fields[5])
		}

		// Optional aliases payload: name\tdefinition\tname\tdefinition...
		// Sent by the zsh plugin so the daemon can suggest shell aliases.
		if len(fields) >= 7 && fields[6] != "" {
			state.SetAliases(parseAliasesPayload(fields[6]))
		}

		return app.List(buffer, selected, promptCol, renderMode, workingDir)
