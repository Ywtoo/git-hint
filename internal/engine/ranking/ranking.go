...
func RankSuggestionsWithReader(commandName string, suggestions []core.CommandMatch, reader io.Reader) ([]core.CommandMatch, error) {
	...
	slices.SortFunc(suggestions, func(a core.CommandMatch, b core.CommandMatch) int {
		...
		// Within a placeholder group the header (Name == "") must come
		// before its items no matter what usage counts say — otherwise the
		// limit step re-inserts a synthetic header above a ranked item and
		// the group renders with a duplicated label.
		aHeader := a.Name == ""
		bHeader := b.Name == ""
		if aHeader != bHeader {
			if aHeader {
		...
		aFlag := strings.HasPrefix(a.Name, "-")
		bFlag := strings.HasPrefix(b.Name, "-")
		if aFlag != bFlag {
			if aFlag {
				return 1
			}
