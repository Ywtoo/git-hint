...
// CommandDataPath returns the full path to a local override/generated JSON.
// Fig specs are kept separately under data/fig and are embedded at build time.
func CommandDataPath(commandName string) (string, error) {
	...
	if err := os.MkdirAll(localDir, 0755); err != nil {
		return "", fmt.Errorf("failed to create local data dir: %w", err)
	}
	return filepath.Join(localDir, commandName+".json"), nil
}
